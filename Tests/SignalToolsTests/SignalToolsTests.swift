...
@Test func testFloatAverageExtensionCorrectnessAndPerformance() async throws {
    ...
    assert(avg == baselineAvg)
}

@available(macOS 15.0, *)
@Test func testFloatNormalizeAndStdDevExtensionsCorrectnessAndPerformance() async throws {
    let t0_floatGen = Date.timeIntervalSinceReferenceDate
    _ = randomFloatData[0]
    let t1_floatGen = Date.timeIntervalSinceReferenceDate
    print("Float array generation took: \(t1_floatGen - t0_floatGen) s")
    ...
    assert(stdDev == baselineStdDev)
}

@available(macOS 15.0, *)
@Test func testFloatNormalize() async throws {
    let t0_floatGen = Date.timeIntervalSinceReferenceDate
    _ = randomFloatData[0]
    let t1_floatGen = Date.timeIntervalSinceReferenceDate
    print("Float array generation took: \(t1_floatGen - t0_floatGen) s")
    ...
    assert(normalized == baselineNormalized)
}
